...
The final path is greatly depends on the global path since the approximation only changes the non-admissible parts with the help of a steering method. Because of the random nature of the RTR global planner the resulted paths could be far from a path which is planned by a human driver. To improve the global paths, it is necessary to define which properties describe a path and to assign a measure to these properties.  In our paper the following parameters were used:

\begin{itemize}
	\item Total length of a path
	\item Number of segments
	\item Number of cusps
	\item Absolute turning angle along the path
	\item Distance to the obstacles
\end{itemize}

A good path could be characterised with low values in the first for cases and with a high value in the last case.

Since during the RTR planning there is no freedom to direct the growth of the trees along these parameters, thus we had to repeatedly run the planning algorithm with different random seed, calculate each time the above presented parameters and at the end choose the path with the best qualities. To normalize the parameters all values should be divided with the maxima of each.
