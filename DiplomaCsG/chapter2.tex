...
A módszert Nagy Ákos dolgozta ki differenciális robotokhoz, majd módosítottam autószerû robotok számára \cite{Akos}. Az elveket végül úgy alakítottuk ki közösen, hogy az ne függjön a robot típusától, így az alkalmazható legyen bármilyen kerekeken mozgó robottípusra.
...
Azokat az eseteket külön jelöljük, amikor a robot egyik kerekére vonatkozó mennyiségekrõl beszélünk. Elsõ indexben azt, hogy elsõ ($f$) vagy hátsó ($r$) kerék, utána, hogy bal ($l$), jobb ($r$), kerékrõl van szó. Ezenkívül a kerekeknél megkülönböztetjük, hogy tangenciális ($a_{t}$), centripetális ($a_{c}$) vagy eredõ ($a$) gyorsulásról beszélünk.
...
\phi^{max}& : \text{A robot maximális kormányszöge, mint fizikai korlátozás}\notag\\
