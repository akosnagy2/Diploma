...
%TODO Sprunk-ra lehetne hivatkozni
...
\par
A pályamenti sebességeket úgy határozzuk meg, hogy a robot gyorsulása a lehetõ legnagyobb legyen. {\Aref{diffRobotAccel}}. egyenlet alapján ezt megtehetjük úgy, hogy a robot kerekeinek tangenciális gyorsulását maximalizáljuk. Több hatás miatt nem tudjuk a kerekek gyorsulását folyamatosan növelni. 
\par
...
a(k) &= \frac{p(k)^4}{4 \cdot \Delta s(k)^2} + c(k) \cdot p(k)^2	 \\
b(k) &= -\frac{2 \cdot v(k+1)^2 \cdot p(k)^4} {4 \cdot \Delta s(k)^2} \notag\\
c(k) &= \frac{v(k+1)^4 \cdot p(k)^4}{4 \cdot \Delta s(k)^2} - {a_{max}}^2 \notag \\
\label{eq:backacp3}
0 &= v^c(k)^4 \cdot a(k) + v^c(k)^2 \cdot b(k) + c(k)
\end{align}
...
\begin{enumerate}
  ...
  \item A harmadik hiba a koordináták meghatározásánál keletkezik. \Aref{fig:samplPoint2}. ábrán látható ahogy a keletkezõ pálya levágja az éles kanyarokat . Ez a hatás is mindig távolabbi végpontot okoz.
\end{enumerate}
...
%TODO
%más módszer miért nem jó
\par
%Elsõként tegyük fel, hogy a mintavételezett pálya végpontja távolabb van, mint a 
%geometriai pályáé. 
...
%TODO 
%Lehetne jobb kép, kisebb mintavételi idõvel talán
\begin{figure}[H]
\centering
...
%TODO
%konkrét eredmények, delta_s_corr értéke mitõl függ.
\par
Szintén fontos megjegyezni, hogy mivel a tárgyalt három hatás elsõsorban negatív $\Delta_{s_{corr}}$-t eredményezz, így a gyakorlatban ez az eset fordul elõ. A gyakorlatot tekintve még megemlítendõ, hogy a $\Delta_{s_{corr}}$ nagyságrendje igen csekély a pálya teljes hosszához képest, nehezen elképzelhetõ 1\%-ot meghaladó arány.
