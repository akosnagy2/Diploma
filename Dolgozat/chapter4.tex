...
\par
Valamint a kerekek centripetális gyorsulása nem haladhatja meg az elõírt eredõ gyorsulás korlátot, különben a robot kereke megcsúszna. A pálya adott $k$. pontjában a kerekek centripetális gyorsulását a következõképpen számolhatjuk ki:
...
\par
Ezután a kerekek tangenciális gyorsulását \eqref{alr} egyenlet alapján határozhatjuk meg.
...
\par
Eddig a két kerék gyorsulást teljesen függetlenül tárgyaltuk, azonban mindkét gyorsulást nem választhatjuk meg szabadon, mert a pálya görbülete meghatározza a köztük lévõ arányt. Ezt a következõképpen láthatjuk be (\eqref{diffRobotWheel} alapján könnyedén belátható, hogy sebességek aránya is ugyanez lesz):
...
\par
Most vizsgáljuk meg, hogy ha a kerékgyorsulás egy adott korlátot megsért, akkor hogyan kapjuk meg belõle azt a robotsebességet, amely esetében még nem sértjük meg a korlátot. 

\par
Elõször tekintsük a tangenciális gyorsulásra vonatkozó korlátot. \eqref{at} egyenletet fejezzük ki $v(k)$-ra mindkét kerék esetén:
...
\end{align}
ahol a $v^{e}(k)$ jelölés arra utal, hogy a sebesség az eredõ gyorsulásra vonatkozó korlátból adódik.

A \eqref{backacp} egyenletet kifejezhetjük $v^{e}(k)$-re. Ekkor egy negyedfokú egyenletet kapunk, ami a következõképpen épül fel:
...
\caption{A pálya és a mintavételezett sebesség profil differenciális robot esetén.} 
\label{fig:diffPath}
\end{figure}
